...
@php
    ...
    if (request()->get('key') === 'college_id' && request()->get('s')) {
        $college = College::find(request()->get('s'));
    }
@endphp
...
@section('page_title')
    ...
    @else
        {{ __('voyager::generic.viewing') . ' ' . $dataType->getTranslatedAttribute('display_name_plural') }}
    @endif
@endsection
...
@section('page_header')
    <div class="container-fluid">
        ...
        {{-- زر الرجوع للكليات --}}
        @if ($college)
            <a href="{{ route('voyager.colleges.index') }}" class="btn btn-default">
                <i class="voyager-angle-left"></i> الرجوع إلى الكليات
            </a>
        @endif

        {{-- إضافة مادة --}}
        @can('add', app($dataType->model_name))
            <a href="{{ route('voyager.' . $dataType->slug . '.create', $college ? ['college_id' => $college->id] : []) }}"
    ...
    <div class="page-content browse container-fluid">
        ...
        @if ($college)
            <div class="alert alert-info">
                <strong>تنبيه:</strong>
                أنت الآن تدير المواد التابعة لكلية
                <strong>{{ $college->title }}</strong>
            </div>
        @endif
        <div class="row">
