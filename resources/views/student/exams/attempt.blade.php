...
<div class="student-layout">
    <main class="student-content" style="max-width:900px;margin:0 auto;">
        <section class="doctor-portal-panel">
            <div class="doctor-portal-panel-head">
                <h3>{{ $exam->title }}</h3>
                <div id="exam-timer" class="doctor-live-status doctor-live-status-started">--:--</div>
            </div>
            <form id="exam-attempt-form" method="POST" action="{{ route('student.exams.submit', $attempt) }}">
                @csrf
                <div id="question-nav" class="doctor-live-actions" style="flex-wrap:wrap;margin-bottom:16px;"></div>
                <div id="questions-container">
                    @foreach($exam->quizQuestions as $index => $qq)
                        @php $answer = $attempt->answers->firstWhere('exam_quiz_question_id', $qq->id); @endphp
                        <div class="exam-question-card live-session-comment" data-question-index="{{ $index }}" style="display:{{ $index === 0 ? 'block' : 'none' }};">
                            <div class="doctor-portal-meta">سؤال {{ $index + 1 }} من {{ $exam->quizQuestions->count() }} — {{ $qq->points }} درجة</div>
                            <div style="margin:12px 0;font-size:1.05rem;">{{ $qq->question_text_snapshot }}</div>
                            <input type="hidden" name="answers[{{ $index }}][exam_quiz_question_id]" value="{{ $qq->id }}">
                            @if($qq->type_snapshot === 'essay')
                                <textarea name="answers[{{ $index }}][answer_text]" class="form-control essay-answer" rows="6" data-qq="{{ $qq->id }}">{{ $answer?->answer_text }}</textarea>
                ...
                <div class="doctor-live-actions" style="margin-top:20px;">
