<div class="container-fluid">
    ...
    <div class="panel panel-bordered" style="margin-top:20px;padding:15px;">
        <h4>إضافة جلسة</h4>
        <form method="POST" action="{{ route($routeBase . '.sections.meetings.store', $section) }}">
            @csrf
            <div class="row">
                <div class="col-md-2">
                    <label>اليوم</label>
    ...
    <div class="panel panel-bordered" style="margin-top:20px;">
        <div class="panel-body">
            ...
            <form method="POST" action="{{ route($routeBase . '.sections.students.attach', $section) }}" class="row" style="margin-bottom:10px;">
                @csrf
                <div class="col-md-4">
                ...
                <tbody>
                    @forelse($section->students as $student)
                        <tr>
                            ...
                            <td>{{ $student->email }}</td>
                            <td>
                                <form method="POST" action="{{ route($routeBase . '.sections.students.detach', [$section, $student]) }}" style="display:inline-block;">
                                    @csrf
                                    @method('DELETE')
                    ...
                    @foreach($section->meetings as $meeting)
                        <tr>
                            ...
                            <td>
                                ...
                                <form method="POST" action="{{ route($routeBase . '.meetings.destroy', $meeting) }}" style="display:inline-block;">
                                    @csrf
                                    @method('DELETE')
                        ...
                        <tr id="edit-meeting-{{ $meeting->id }}" class="collapse">
                            <td colspan="4" style="background:#f9f9f9;">
                                <form method="POST" action="{{ route($routeBase . '.meetings.update', $meeting) }}">
                                    @csrf
                                    @method('PUT')
                                    <div class="row">
                                        <div class="col-md-2">
