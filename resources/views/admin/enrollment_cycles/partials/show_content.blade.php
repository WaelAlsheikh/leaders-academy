<div class="container-fluid">
    ...
    <div class="panel panel-bordered" style="padding:15px;">
        <h4>تفاصيل الدورة</h4>
        <form method="POST" action="{{ route($routeBase . '.enrollment_cycles.update', $cycle) }}">
            @csrf
            @method('PUT')
            <div class="row">
                <div class="col-md-3">
    ...
    <div class="panel panel-bordered" style="margin-top:20px;padding:15px;">
        ...
        <div style="display:flex;gap:10px;flex-wrap:wrap;">
            <form method="POST" action="{{ route($routeBase . '.enrollment_cycles.open', $cycle) }}">
                @csrf
                <button type="submit" class="btn btn-success">فتح التسجيل</button>
            </form>
            <form method="POST" action="{{ route($routeBase . '.enrollment_cycles.close', $cycle) }}">
                @csrf
                <button type="submit" class="btn btn-warning">إغلاق التسجيل</button>
            </form>
            <form method="POST" action="{{ route($routeBase . '.enrollment_cycles.approve', $cycle) }}">
                @csrf
                ...
                <tbody>
                    @foreach($semesters as $semester)
                        <tr>
                            ...
                            <td>
                                <a class="btn btn-sm btn-success" href="{{ route($routeBase . '.semesters.sections.index', $semester) }}">
                                    إدارة الشعب
                                </a>
                            </td>
                        </tr>
    ...
    <div class="panel panel-bordered" style="margin-top:20px;padding:15px;">
        ...
        @else
            <form method="POST" action="{{ route($routeBase . '.enrollment_cycles.start_semester', $cycle) }}">
                @csrf
                <div class="row">
                    <div class="col-md-4">
        ...
        <div class="panel-body">
            ...
            <form method="GET" action="{{ route($routeBase . '.enrollment_cycles.show', $cycle) }}" class="row" style="margin-bottom:10px;">
                ...
                <div class="col-md-3" style="margin-top:25px;">
                    ...
                    <a href="{{ route($routeBase . '.enrollment_cycles.show', $cycle) }}" class="btn btn-default">إزالة</a>
                </div>
            </form>
